...
// DNS-over-HTTPS 代理 - Cloudflare Pages Functions 版本
...
// 向特定DNS服务器发送查询
async function queryDnsServer(request, originalUrl, dnsServer) {
  // 构建目标 DoH 请求
...
export async function onRequest(context) {
  ...
  try {
    ...
    for (const dnsServer of DNS_SERVERS) {
      try {
        const response = await queryDnsServer(request, url, dnsServer);
